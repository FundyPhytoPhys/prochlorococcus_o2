...
```{r, ProchlorococcusEnzymeKmsTrim, fig.cap = ProchlorococcusEnzymeKmsTrim_cap, fig.width = 20, fig.height = 10}
...
ProchlorococcusEnzymeKmsTrim <- MergedData %>%
  filter(!is.na(Gene_count),
         ...
         !is.na(Estimated),
         Taxa == "Prokaryote",
         EnzymeName %in% c("decaprenylphospho-beta-D-ribofuranose 2-dehydrogenase", "ubiquinol oxidase (non-electrogenic)","acireductone dioxygenase (Ni2+-requiring)", "pyridoxal 5'-phosphate synthase", "(S)-2-hydroxy-acid oxidase", "bacterial non-heme ferritin")) %>% 
  mutate(StrainLabel = case_when(Strain == "CCMP1375" ~ "SS120",
                                 Strain == "CCMP1986" ~ "MED4",
                                 Strain == "MIT9313" ~ "MIT9313",
                                 Strain == "NATL2A" ~ "NATL2A")) %>%
 ...
 ProchlorococcusEnzymeKmsTrim %>% 
  ...
  geom_point(aes(x = reorder(EnzymeName_bold, Km_mM* 1000), y = (Km_mM* 1000), shape = as.logical(Estimated), size = Gene_count), stroke = 1) +
  geom_vline(aes(xintercept = reorder(EnzymeName_bold, Km_mM* 1000)), size = 2,  data = . %>% filter(is.na(Km_mM)), linetype = "solid") +
  geom_text(aes(x = 7, y = 2.5, label = "[O2] ≈ 2.5 µM"), color = "blue", size = 6) +
  geom_text(aes(x = 7, y = 25, label = "[O2] ≈ 25 µM"), color = "darkgreen", size = 6) +
  geom_text(aes(x = 7, y = 250, label = "[O2] ≈ 250 µM"), color = "#FF0000", size = 6) +
  ggh4x::facet_nested(rows = vars(StrainLabel)) +
  theme_bw() +
   ggpubr::rotate_x_text(angle = 80) + 
   scale_shape_manual(values = c(19,8), labels = c("FALSE" = "Measured", "TRUE" = "Inferred")) +
