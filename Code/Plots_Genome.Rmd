...
(ProchlorococcusEnzymeKms <- MergedData %>%
  ...
  ungroup() %>% 
  ggplot() +
  geom_errorbar(aes(x = reorder(EnzymeName, Km_mM * 1000), ymin = Km_mM_min* 1000, ymax = Km_mM_max* 1000)) +
  ...
  geom_point(aes(x = reorder(EnzymeName, Km_mM* 1000), y = (Km_mM* 1000), shape = as.logical(Estimated), size = Gene_count), stroke = 1) +
