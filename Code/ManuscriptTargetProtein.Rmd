...
MIT9313ProteinAnnotated <- TargetProtein %>%
  filter( Strain == "MIT9313") %>% 
   mutate(WL_label = case_when(WL %in% c(450, 530, 660) ~ paste(WL, "nm", sep = " "),
                              WL %in% c("WW") ~ paste("White LED", sep = " ")),
        WL_label = factor(WL_label, levels= c("White LED", "660 nm", "530 nm", "450 nm")),
         Oxygen_Label = case_when(O2_uM %in% c(250, 25, 2.5) ~ paste(O2_uM, "µM", sep = " ")),
         Par_Label = case_when(Par_ue %in% c(30, 90, 100, 180) ~ paste(Par_ue, "µE", sep = " "))) %>% 
 ...
 labs(y = "Concentration" ~  " (fmole µg"  ~ protein^-1~")", x = "Protein") +
  # scale_fill_manual(fill = (O2_uM)) %>% 
  # scale_colour_manual(values = ColourOxygen) +
 ggh4x::facet_nested(cols = vars(Strain, Oxygen_Label), rows = vars(WL_label, Par_Label), 
                    labeller = labeller(Par_ue = label_both, WL = label_both, O2 = label_both, Strain = label_value)) +
 # guides(fill = "none") +
  theme_bw()
