...
MIT9313ProteinAnnotated <- TargetProtein %>%
  filter( Strain == "MIT9313") %>% 
   mutate(WL_label = case_when(WL %in% c(450, 530, 660) ~ paste(WL, "nm", sep = " "),
        ...
        Oxygen_Label = factor(Oxygen_Label, levels = c("250 µM", "2.5 µM")),
        Par_Label = factor(Par_Label, levels = c("30 µE", "90 µE", "100 µE", "180 µE"))) %>% 
 ...
 ggh4x::facet_nested(cols = vars(Strain, Par_Label ), rows = vars(Oxygen_Label), 
