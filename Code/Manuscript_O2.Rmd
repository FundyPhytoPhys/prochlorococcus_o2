...
Data files saved from the Multicultivator software were imported into R-Studio [@R-Studio] for data management [@R-tidyverse], growth rate calculations CITATION, comparisons of model fits CITATION, and generation of
...
relationship between chlorophyll-specific µ, d^-1^, for each [O~2~] level, across the blue and red spectral wavebands, photoperiods and PAR levels for each *P.marinus* strain in this study. The gam function from the R package `mgcv`
[@R-mgcv] was used to model the growth rate with smoothing terms to
indicate the 90, 50 and 10% quantiles for growth rate across the levels of factors. Growth rates with a standard error tolerance below 30% of the fitted growth rate was used in the model. Our priority
...
deep ocean niches.  We included analyses of responses of red light, which is not ecophysiologically relevant but might prove mechanistically informative [@murphyPhotoinactivationPhotosystemII2017].

### Estimation of Photosynthetically Usable Radiation {.unnumbered}

To calculate the Photosynthetic Usable Radiation (PUR), a proxy of photons absorbed, for each ecotype
of *P. marinus*, the imposed Photosynthetic Active
Radiation (PAR) was first determined. An emission profile of each colored LED
...
Ltd, Malvern, UK) and scanned to measure the emission wavelengths from 200 nm to 1000 nm. Following the determination of PAR, an *in-vivo* whole cell absorbance spectrum for each *P. marinus* was obtained using the Olis 14 UV/VIS Clarity
Spectrophotometer (Olis Inc, Bogart, Georgia) by scanning across the visible spectral range of λ = 350 nm to 750 nm at 1 nm intervals. The 20 cm path length of the internally reflective cavity of the Olis spectrophotometer was corrected to a 1 cm path length using the Javorfi correction method [@javorfiQuantitativeSpectrophotometryUsing2006] on PRO 99 media subtracted whole cell absorbance spectra. Whole cell absorbance spectra were measured for each strain under each imposed spectral waveband.

A weighted PUR spectrum was determined by applying an integrated weighted function to the blank subtracted, whole cell absorbance spectrum for each *P. marinus* ecotype and the emission spectrum of the imposed LED growth light [@morelAvailableUsableStored1978]. The whole cell absorbance spectrum was normalized to the absorbance maximum of divinylchlorophyll a (Chl a~2~), determined dynamically between 400 nm and 460 nm, and each LED spectrum was normalized to the emission maximum.

Equation \@ref(eq:WeightedPUR) was used to determine the weighted PUR spectrum P(λ) according to [@morelAvailableUsableStored1978],
...
where P(λ) is the weighted PUR absorbance spectrum over 400 nm to 700 nm spectral range, A(λ) is the Chl a~2~ normalized whole cell absorbance spectrum over 400 nm to 700 nm and E(λ) is the imposed growth light emission spectrum over 400 nm to 700 nm.
...
where P(λ) is the weighted PUR absorbance spectrum from equation \@ref(eq:WeightedPUR), E(λ) is the imposed growth light emission spectrum from equation \@ref(eq:WeightedPUR) and PAR is the imposed light level (µmol photons m^-2^ s^-1^).  Figure (\@ref(fig:PurParPlot)) shows the calculated absorbed PUR (µmol photons m^-2^ d^-1^) versus imposed actinic PAR (µmol photons m^-2^ s^-1^) for each strain and each spectral waveband (nm).


The applied photoperiod was administered using a sinusoidal function to simulate light levels equivalent to sun rise and sunset. The area under a sine curve is equivalent to a triangle, therefore the equation to determine the cumulative diel absorbed PUR (µmol photons m^-2^ d^-1^) is one half of the base (photoperiod) multiplied by the height (PUR) equation \@ref(eq:SumPURDay).
...
### Protein Immunoblot {.unnumbered}

XXX Mireille to confirm and summarise XXX
Samples were thawed and placed in FastPrep lysing vials with 500 uL of 1X Protein Extraction Buffer (PEB). Cells were lysed for 3 cycles of 60 seconds at 6.5m s-1 with 60 second intervals on ice between cycles.  The lysing vials were centrifuged at 16100g for 5 minutes, the supernatant was transferred to a 600 uL microcentrifuge tube and centrifuged again at 16100g for 3 minutes.  40uL of the supernatant were subsampled into labeled vials and stored in  -80 oC freezer until analysis. The bicinchoninic acid assay (BCA) was used for total protein determination using a calibration curve of bovine gamma globulin (BGG) standard in the 0 to 1.0 mg/mL range.

All samples and appropriate standards were prepared as described in [@brownFluxCapacitiesAcclimation2008] and loaded onto either 15 or 17 well Invitrogen Bolt 4-12% Bis-Tris gels.  All sample extract loads were calculated based on 2.0 ug of total protein.  All gels were run for 25 minutes at 200V and transferred for 60 minutes at 20V onto a PVDF membrane.  Blots were blocked overnight in 2% ECL blocking solution made with 1XTBS-T, the exception being target protein Fcp6 which was not blocked.  Fcp6 was incubated overnight with 1:5000 dilution of primary antibody (obtained by J. Lavaud) in 2% ECL blocking solution directly after transfer.  The following morning, each blot was incubated in their appropriate antibody and agitated gently on a rotator table for 1 hour at room temperature.   Dilution of primary antibodies was  PsbA 1:10000 (Agrisera,  AS05 084), and RbcL 1:10000 (Agrisera,  AS03037).   Each blot was washed with 10mL of 1XTBS-T. Two times briefly preceded by 15 minutes on a rotator table, followed by three times for 5 minutes on the rotator table.  All blots were incubated in a 1:15000 dilution of secondary antibody (anti-rabbit HRP Biorad lot 64230791) and agitated gently on a rotator table for 1 hour at room temperature.   All blots were washed as per the wash protocol described above.  Blots were incubated in Amersham ECL Select chemiluminescent substrate and imaged in a Biorad VersaDoc imager.

Target protein standards were loaded at 3 concentrations to establish a calibration curve.  PsbA standard (Agrisera, AS01 016S) range 12.5 - 100 fmol per lane and RbcL standard (Agrisera, AS01 017S) range 175 - 1400 fmol per lane. 
...
derived from HL ecotypes of *P. marinus* in OMZ at depths up to 200
...
### MED4 PAR and growth trends {.unnumbered}

#### High oxygen environment {.unnumbered}

The growth rate of MED4, from the high light (HLI) clade of *P.
marinus*, increased with higher imposed PAR and longer photoperiods
...
Similar to the growth trends of the 250 µM O~2~ experiments, MED4
maintained at 25 µM O~2~, showed the fastest growth rate when the
...
generally similar growth rate as the 250 µM O~2~ growth experiments,
...
MED4 did not grow under any tested conditions sparged to the lowest O~2~
of 2.5 µM (Figure \@ref(fig:MED4PAR)). 2.5 µM O~2~ growth experiments
were not conducted for 4 and 16 h photoperiods, as no growth occurred
...
### SS120 PAR and growth trends {.unnumbered}

#### High oxygen environment {.unnumbered}

The growth rate of SS120, from the low light (LLIII) clade of *P.
marinus*, increased with lower imposed PAR and longer photoperiods, when
...
only when the photoperiod was longer than 4 h under blue, white, and red
light. SS120 required longer than 8 h photoperiod of imposed green
light. No growth was observed under any blue light photoperiods when
exposed to PAR of 90 µmol photons m^-2^ s^-1^. Growth rate increased
with increasing photoperiods for white, red and green light when exposed
...
of 180 µmol photons m^-2^ s^-1^ showed no growth under imposed blue and
...
photons m^-2^ s^-1^ and white LED light. The growth rate generally
increased with longer photoperiods under green light treatments, however
no growth was observed under the 8 h photoperiod. The µ~max~ was
...
photons m^-2^ s^-1^ experiments, SS120 maintained at 25 µM O~2~, no
growth was observed under a 4 h photoperiod for imposed red spectral
...
photoperiod. This growth rate decreased by 53% compared to the 8 h blue
light photoperiod under PAR of 90 µmol photons m^-2^ s^-1^. The µ~max~
was
...
The 25 µM O~2~, less than 16 h photoperiod and 180 µmol photons m^-2^ s^-1^ experiments under white LED
light were not performed due to time constraints.

#### Low oxygen environment {.unnumbered}

SS120 only grew
(`r PURPARFits %>% filter(Strain == "SS120", WL == "660", O2_uM == "2.5", Photoperiod == 12) %>% select(ReplicateMean_deltaOD_Lmu_corr_day) %>% max() %>% round(digits = 2)`
d^-1^) under PAR of 30 µmol photons m^-2^ s^-1^ and 12 h red light
photoperiod when sparged to the lowest O~2~ of 2.5 µM (Figure
\@ref(fig:SS120PAR)). SS120 did not grow under any other tested
conditions. 2.5 µM O~2~ growth experiments were not conducted for 4 and
...
light 16 h photoperiod experiments were not performed due to time
constraints.
...
\newpage
### MIT9313 PAR results {.unnumbered}

#### High oxygen environment {.unnumbered}

The growth rate of MIT9313, from the low light (LLIV) clade of *P.
marinus*, increased with low to moderate imposed PAR and longer
...
`r PURPARFits %>% filter(Strain == "MIT9313", WL == "450", O2_uM == "25", Photoperiod == 16, Par_ue == 90) %>% select(ReplicateMean_deltaOD_Lmu_corr_day) %>% max() %>% round(digits = 2)`
...
µ~max~ was
`r PURPARFits %>% filter(Strain == "MIT9313", WL == "450", O2_uM == "2.5", Photoperiod == 4, Par_ue == 180) %>% select(ReplicateMean_deltaOD_Lmu_corr_day) %>% max() %>% round(digits = 2)`
