...
```{r create maps, fig.height = 8, fig.width = 10, warning = FALS}
...
# PhotoperiodDepthFullO2$facetsFull <- 'Full Oxygen'
# PhotoperiodDepthOMZ$facetsOMZ <- 'Oxygen Minimum Zone'
...
mutate(TextLabel = (case_when(CladeHLI == 1 & CladeLLII_LLIII == 0 & CladeLLIV == 0 ~ "HLI",
        ...
        TextLocation = 0) %>% 
ggplot()+
  geom_point(aes(x = PhoticperiodPeakSatPAR_h, y = DepthNames), size = 0.5, alpha = 0, show.legend = TRUE)+
  geom_text(aes(x = TextLocation, y = DepthNames, label = paste(TextLabel), show.legend = FALSE), hjust = 0, size = 6)  +
  labs(title = "Full Oxygen") +
  scale_x_continuous(breaks=seq(0, 25, by = 5)) +
  coord_fixed(ratio = 2, xlim = NULL, ylim = NULL, expand = TRUE, clip = "on") +
  ggh4x::facet_nested(cols = vars(DayofYear), labeller = labeller(lat = label_both, DayofYear = label_value)) +
  guides(color = "none") +
  theme_bw()+
...
# PhotoperiodDepthOMZ1<-PhotoperiodDepthOMZ %>% 
# filter(DayofYear == "Summer Solstice")
# PhotoperiodDepthOMZ1$nm <- c(400, 400, 700, 700)
# PhotoperiodDepthOMZ1$de <- c(400, 400, 700, 700)
# 
# PhotoperiodDepthOMZ2<-PhotoperiodDepthOMZ %>% 
# filter(DayofYear == "Winter Solstice")
# PhotoperiodDepthOMZ2$nm <- c(400, 400, 700, 700)
# 
# PhotoperiodDepthOMZ3<-PhotoperiodDepthOMZ %>% 
# filter(DayofYear == "Equinox")
# PhotoperiodDepthOMZ3$nm <- c(400, 400, 700, 700)
# 
# PhotoperiodDepthOMZss<-rbind(PhotoperiodDepthOMZ1, PhotoperiodDepthOMZ2, PhotoperiodDepthOMZ3)
...
ggplot()+
  # stat_wl_strip(aes(x = nm), ymin = "50 m", ymax = "100 m", alpha = 0.5, PhotoperiodDepthOMZss) +
  # scale_fill_identity() +
  geom_point(aes(x = PhoticperiodPeakSatPAR_h, y = DepthNames), size = 0.5, alpha = 0, show.legend = TRUE)+
  geom_text(aes(x = TextLocation, y = DepthNames, label = paste(TextLabel), show.legend = FALSE), hjust = 0, size = 6)  +
  scale_x_continuous(breaks=seq(0, 25, by = 5)) +
...
data_text1 <- data.frame(label = c('~Clade~HLI'))
data_text2 <- data.frame(label = c('~Clade~HLI'))
data_text3 <- data.frame(label = c('~Clade~HLI'))
...
RainbowTropicFull<-ggplot() +
  stat_wl_strip(aes(x = nm), ymin = 180, ymax = 200, alpha = 0.5, world_coordinatesRainbow) +
  scale_fill_identity() +
