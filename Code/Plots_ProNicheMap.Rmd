...
ggplot()+
  ...
  labs(title = "Full Oxygen") +
  scale_x_continuous(breaks=seq(0, 25, by = 5)) +
      coord_fixed(ratio = 1.7, xlim = NULL, ylim = NULL, expand = TRUE, clip = "on") +
    facet_grid(cols = vars(DayofYear),
              labeller = labeller(lat = label_both, DayofYear = label_value)) +
   geom_text(aes(x = TextLocation, y = DepthNames, label = paste(TextLabel), show.legend = FALSE ),
    size = 6
  )  +
  guides(color = "none") +
...
# ggarrange(P1, map2, P2, ncol = 3, nrow = 1)

```


```{r save plot}
ggsave(file = file.path(FigPath, paste("NichesMap_UglySS2",".png",sep = "")), height=12, width= 28,  dpi = 300, limitsize = TRUE)
```
