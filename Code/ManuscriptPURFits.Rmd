...
# geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, linetype = as.factor(Photoperiod)), size = 1, show.legend = F) +
# geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, linetype = as.factor(Data_all_WL)), size = 1, show.legend = F) +
  geom_rect(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, fill = "white") +
  geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 3, alpha = 1.0) +
    geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 1.0, alpha = 0.8) +
 ...
 labs(y = "Chlorophyll Specific Growth Rate " ~  " ("  ~ d^-1~")", x = "PUR (µmol photons" ~m^-2 ~d^-1~")") +
  ...
  scale_linetype_manual(values = c("solid", "dashed"), labels = c("p value < 0.05", "p value > 0.05")) +
  scale_shape_manual(values = PhotoperiodShapes, labels = c("16 h", "12 h", "8 h", "4 h") )  +
  ggh4x::facet_nested(rows = vars(OxygenLabel, O2_uM), cols = vars(Strain), 
                    labeller = labeller(Strain = label_value, O2_uM = label_value, OxygenLabel = label_value)) +
 guides(
...
ggsave(file = file.path(PlotsOut, paste("BluevsRedPurFitsPlots",".png",sep = "")), plot = BluevsRedPurFitsPlots, device = NULL, scale = 1, height=30, width= 44, units = c("cm"),dpi = 300, limitsize = TRUE)

# ggsave(file = file.path(PlotsOut, paste("BluevsRedPurFitsPlots",".tiff",sep = "")), plot = BluevsRedPurFitsPlots, device = NULL, scale = 1, height=30, width= 44, units = c("cm"),dpi = 300, limitsize = TRUE)
