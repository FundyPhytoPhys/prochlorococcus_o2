...
PURPARStrainO2PhotoNestModels_filtered <- PURPARFits_dummy |>
   filter(PURPhotonDose_day <= 1.5e+06) |>
 nest(data = -c("Strain","O2_uM", "Photoperiod")) |>
  mutate(GRCplatt_Model = map(data, possibly(~nlsLM(deltaOD_Lmu_corr_day ~ grcplatt(I = PURPhotonDose_day, a, b, Pmax),
                              data = .x,
                              start = list(a = 1e-8, b = 1e-9, Pmax = 1),
...
#   geom_line(aes(x = PURPhotonDose_day, y = `.fitted`)) + 
#   facet_grid(rows = vars(O2_uM), cols = vars(Strain)) + 
#     labs(title = "PURPARStrainO2NestModels GRC") +
#   theme_bw()
...
# PURPARStrainO2NestModels |>
#   filter(!map_lgl(GRCplatt_Model, is.null)) |> # only include fits that did not fail
#   unnest(GRCplatt_Predict, data) |>
#    mutate(O2_uM = factor(O2_uM, levels = c(250, 25, 2.5)),
...
#     labs(title = "PURPARStrainO2PhotoNestModels GRC") +
#   theme_bw()
...
PURPARStrainO2PhotoNestModels_filtered |>
  filter(!map_lgl(GRCplatt_Model, is.null)) |>
  unnest(GRCplatt_Predict, data) |>
   mutate(O2_uM = factor(O2_uM, levels = c(250, 25, 2.5)),
         Strain = factor(Strain, levels = c("MED4", "SS120", "MIT9313")),
  ...
  facet_grid(rows = vars(O2_uM, Photoperiod), cols = vars(Strain)) +
    labs(title = "PURPARStrainO2PhotoNestModels_filtered Platt") +
  theme_bw()
...
PURPARStrainO2PhotoNestModels_filtered |>
  ...
  geom_line(aes(x = PURPhotonDose_day, y = `.fitted`)) +
   scale_colour_manual(values = MCMIXColours) +
  facet_grid(rows = vars(O2_uM, Photoperiod), cols = vars(Strain)) +
    labs(title = "PURPARStrainO2PhotoNestModels_filtered Linear Model") +
  theme_bw()
...
#     labs(title = "PURPARStrainO2WLNestModels Platt") +
#   theme_bw()
...
#     labs(title = "PURPARStrainO2PeriodWLNest Platt") +
#   theme_bw()
...
# PURPARStrainO2BestModels <- PURPARStrainO2NestModels|>
#   # select(-c(data)) |>
...
# PURPARStrainO2BestModels_filtered <- PURPARStrainO2NestModels_filtered|>
#   # select(-c(data)) |>
#   pivot_longer(cols = -c(Strain, O2_uM, data)) |>
#   drop_na(value) |>
#   filter(str_detect(name, pattern = "Model") | str_detect(name, pattern = "Glance") | str_detect(name, pattern = "Param")) |>
...
#   pivot_wider(names_from = term, values_from = c(estimate, std.error, p.value))
#  
# PURPARStrainO2BestModels_filtered
# 
# PURPARStrainO2PhotoBestModels_filtered <- PURPARStrainO2PhotoNestModels_filtered|>
#   # select(-c(data)) |>
...
#   pivot_wider(names_from = term, values_from = c(estimate, std.error, p.value))
# 
# PURPARStrainO2PhotoBestModels_filtered





# PURPARStrainO2PhotoBestModels <- PURPARStrainO2PhotoNestModels|>
#   # select(-c(data)) |>
#   pivot_longer(cols = -c(Strain, O2_uM, Photoperiod, data)) |>
#   drop_na(value) |>
#   filter(str_detect(name, pattern = "Model") | str_detect(name, pattern = "Glance") | str_detect(name, pattern = "Param")) |>
...
#   pivot_wider(names_from = term, values_from = c(estimate, std.error, p.value))
#  
# PURPARStrainO2PhotoBestModels


# PURPARStrainO2WLBestModels <- PURPARStrainO2WLNestModels|>
#   # select(-c(data)) |>
...
#   pivot_wider(names_from = term, values_from = c(estimate, std.error, p.value))
#  
# PURPARStrainO2WLBestModels



# PURPARStrainO2PhotoWLBestModels <- PURPARStrainO2PhotoNestModels|>
#   select(-c(data)) |>
...
#   pivot_wider(names_from = term, values_from = c(estimate, std.error, p.value))
#  
# PURPARStrainO2PhotoWLBestModels
...
KeepPlattModel <- PURPARStrainO2PhotoNestModels_filtered|>
...
KeepLmModel <- PURPARStrainO2PhotoNestModels_filtered|>
  filter(map_lgl(GRCplatt_Model, is.null)) |>
...
StrainO2Photoperiod_AllModels <- full_join(KeepPlattModel, KeepLmModel) 
 ...
 StrainO2Photofiltered_PlattLmModel <- StrainO2Photoperiod_AllModels |>
   pivot_longer(cols = -c(Strain, O2_uM, Photoperiod, data)) |>
  drop_na(value) |>
...
StrainO2_AllModels <- full_join(KeepPlattModel, KeepLmModel) 
 ...
 # remove(PURPARStrainO2PhotoNestModels_filtered)
...
# PURPARStrainO2WLBestModels <- PURPARStrainO2WLBestModels|>
#   mutate(estimate_a = if_else(ModelType == "GRClm", 0, estimate_a),
#          estimate_b = if_else(ModelType == "GRClm", 0, estimate_b),
#          estimate_Pmax = if_else(ModelType == "GRClm", 0, estimate_Pmax),
#          AIC = if_else(ModelType == "GRClm" & is.infinite(AIC), 0, AIC)) 
...
StrainO2Photofiltered_PlattLmModel <- StrainO2Photofiltered_PlattLmModel|>
  mutate(estimate_a = if_else(ModelType == "GRClm", 0, estimate_a),
         estimate_b = if_else(ModelType == "GRClm", 0, estimate_b),
         estimate_Pmax = if_else(ModelType == "GRClm", 0, estimate_Pmax),
         AIC = if_else(ModelType == "GRClm" & is.infinite(AIC), 0, AIC)) 
 
StrainO2Photofiltered_PlattLmModel
...
StrainO2filtered_PlattLmModel |>
  ...
  facet_grid(rows = vars(O2_uM), cols = vars(Strain)) +
    labs(title = "Single fit for each [O2]- all photoperiods included that had growth") +
  theme_bw()
...
StrainO2Photofiltered_PlattLmModel |>
  ...
  geom_line(aes(x = PURPhotonDose_day, y = `.fitted`)) + 
   geom_text(aes(x = 500000, y = 0.75, label = paste(ModelType), show.legend = FALSE)
     ...
     scale_colour_manual(values = MCMIXColours) +
  facet_grid(rows = vars(O2_uM, Photoperiod), cols = vars(Strain)) +
    labs(title = "Fits to be used for ANOVA comparison to single fit in each [O2]") +
  theme_bw()
...
Wider_Photoperiods <- StrainO2Photofiltered_PlattLmModel %>% 
...
```
# Remove the photoperiods from the model that are significantly different from the ANOVA run on the single fit against each individual photoperiod.   
```{r}
# model with dummy dark, 0 PAR,  growth rates. Also removed data if no growth occurred for an O2 and photoperiod combination so that the model including all photoperiods would not be skewed towards zero. As per Amanda and Doug's advice, filtered out PUR d-1 above 1.5e+06 because only photoperiods greater than 4 hrs have PUR d-1 above.
PURPARStrainO2NestModels_filteredPhotoperiods <- PURPARFits_dummy |>
  mutate(remove = case_when(Strain == "MED4" & Photoperiod == 4 ~ 1,
                            Strain == "MED4" & O2_uM == 25 & Photoperiod == 8 & WL == "WW" ~ 1,
                            Strain == "MIT9313" & O2_uM == 2.5 & Photoperiod == 16 ~ 1,
                            Strain == "MIT9313" & O2_uM == 2.5 & Photoperiod == 12 & WL == "WW" ~ 1,
   ...
   filter(remove == 0,
          PURPhotonDose_day <= 1.5e+06) |>
  mutate(removephotoperiod =  case_when(Strain == "MED4" & Photoperiod == 4 ~ 1,
                                        Strain == "MED4" & O2_uM == 250 & Photoperiod == 8 ~ 1,
                                        Strain == "MED4" & O2_uM == 25 & Photoperiod == 16 ~ 1,
                                        Strain == "MIT9313" & Photoperiod == 4 ~ 1,
                                        Strain == "MIT9313" & O2_uM == 25 & Photoperiod %in% c(8,12) ~ 1,
                                        Strain == "MIT9313" & O2_uM == 2.5 & Photoperiod %in% c(12,16) ~ 1,
                                        Strain == "SS120" & Photoperiod == 4 ~ 1,
                                        Strain == "SS120"  & O2_uM == 250 & Photoperiod %in% c(8,12) ~ 1,
                                        Strain == "SS120"  & O2_uM == 25 & Photoperiod %in% c(8) ~ 1,
                                        Strain == "SS120"  & O2_uM == 2.5 & Photoperiod %in% c(8) ~ 1,
                                        TRUE ~ 0))  |>
  nest(data = -c("Strain","O2_uM")) |>
  mutate(GRCplatt_Model = map(data, possibly(~nlsLM(deltaOD_Lmu_corr_day ~ grcplatt(I = PURPhotonDose_day, a, b, Pmax),
                              data = .x,
...
```{r}
 ...
 #filter to keep platt models
KeepPlattModel <- PURPARStrainO2NestModels_filteredPhotoperiods|>
  select(-c(GRClm_Glance, GRClm_Model, GRClm_Param, GRClm_Predict)) |>
   filter(!map_lgl(GRCplatt_Model, is.null))

#filter to keep only the models that failed to fit a platt model, fits by lm remains. 
KeepLmModel <- PURPARStrainO2NestModels_filteredPhotoperiods|>
  filter(map_lgl(GRCplatt_Model, is.null)) |>
 select(-c(GRCplatt_Glance, GRCplatt_Model, GRCplatt_Param, GRCplatt_Predict))
...
StrainO2_AllModels <- full_join(KeepPlattModel, KeepLmModel) 
 ...
 StrainO2filteredPhotoperiod_PlattLmModel <- StrainO2_AllModels|>
  # select(-c(data)) |>
  pivot_longer(cols = -c(Strain, O2_uM, data)) |>
  drop_na(value) |>
  filter(str_detect(name, pattern = "Model") | str_detect(name, pattern = "Glance") | str_detect(name, pattern = "Param") | str_detect(name, pattern = "Predict")) |>
...
StrainO2filteredPhotoperiod_PlattLmModel |>
  unnest(Predict, data) |>
  mutate(O2_uM = factor(O2_uM, levels = c(250, 25, 2.5)),
         WL= factor(WL, levels = c("WW", 660, 450)),
         Strain = factor(Strain, levels = c("MED4", "SS120", "MIT9313")),
  ...
  geom_line(aes(x = PURPhotonDose_day, y = `.fitted`)) + 
   ...
   scale_shape_manual(values = PhotoperiodShapes)  +
  facet_grid(rows = vars(O2_uM), cols = vars(Strain)) +
    labs(title = "New single fit for each [O2]- photoperiods that were significantly different were removed from single fit") +
  theme_bw()
...
```
# ANOVA on new model with signifiacntly different fits removed from the single fit.
```{r compare 1 fit of all photoperiod to each photoperiods using platt or lm}
...
# this contains 1 fit through all photoperiods, the photoperiods with no growth were filtered out.
AllPhotoperiods <- StrainO2filteredPhotoperiod_PlattLmModel %>% 
  select(Strain, O2_uM, Model) %>% 
 rename(All = 'Model')
...
# ANOVA_Photoperiods <- PURPARStrainO2PhotoNestModels %>% 
...
# # PURPARStrainO2PhotoperiodBestModels <- PURPARStrainO2PhotoNestModels |>
# #   select(-c(data)) |>
