...
AllStrainPurFitsPlots <- AllFitsCombined |>
  ...
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.75, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MED4")) + 
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(O2_uM != 2.5, Strain == "MED4")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(O2_uM != 2.5, Strain == "MED4")) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.75, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "SS120")) + 
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotdash", data = . %>% unnest(Predict_16) %>%  filter(O2_uM == 2.5, Strain == "SS120")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dashed", data = . %>% unnest(Predict_12) %>%  filter(O2_uM == 250, Strain == "SS120")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(Strain == "SS120")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(Strain == "SS120")) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.75, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MIT9313")) + 
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotdash", data = . %>% unnest(Predict_16) %>%  filter(O2_uM == 2.5, Strain == "MIT9313")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dashed", data = . %>% unnest(Predict_12) %>%  filter(O2_uM == 25, Strain == "MIT9313")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(O2_uM != 2.5, Strain == "MIT9313")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(Strain == "MIT9313")) +
...
ggsave(file = file.path(PlotsOut, paste("AllStrainPurFitsPlots",".png",sep = "")), plot = AllStrainPurFitsPlots, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)

ggsave(file = file.path(PlotsOut, paste("AllStrainPurFitsPlots",".tiff",sep = "")), plot = AllStrainPurFitsPlots, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)
...
WLvsPooledPurFitsPlots <- AllFitsCombinedWL |>
  unnest(cols = c(Data_all), names_sep = "_", keep_empty = TRUE) |>
  mutate(PhotoperiodLabel_title = "Photoperiod (h)",
         OxygenLabel = "Oxygen Concentration (µM)",
         Photoperiod = factor(Data_all_Photoperiod, levels = c(16, 12, 8, 4)),
  ...
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 1.0, alpha = 0.3, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MED4")) + 
   geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, data = . %>% unnest(Predict_450) %>%  filter(O2_uM == 2.5, Strain == "MED4",  WL == 450)) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, data = . %>% unnest(Predict_660) %>%  filter(Strain == "MED4", WL == 660))+
    geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, data = . %>% unnest(Predict_WW) %>%  filter(Strain == "MED4", WL == "WW")) +
   ...
   geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5,  data = . %>% unnest(Predict_450) %>%  filter(Strain == "SS120", O2_uM != 25, WL == 450)) +
 geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5,  data = . %>% unnest(Predict_WW) %>%  filter(Strain == "SS120", O2_uM != 250, WL == "WW")) +
  ...
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, data = . %>% unnest(Predict_660) %>%  filter(Strain == "MIT9313", O2_uM != 250, WL == 660))+
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5,  data = . %>% unnest(Predict_WW) %>%  filter(Strain == "MIT9313", O2_uM == 2.5, WL == "WW")) +
...
BluevsRedPurFitsPlots <- AllFitsCombinedWL |>
  ...
  geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 2, alpha = 0.7) +
    geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 1.0, alpha = 0.5) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, data = . %>% unnest(Predict_450) %>%  filter(Strain == "MED4",  WL == 450)) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, data = . %>% unnest(Predict_660) %>%  filter(Strain == "MED4", WL == 660)) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5,  data = . %>% unnest(Predict_450) %>%  filter(Strain == "SS120", O2_uM == 25, WL == 450)) +
 geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5,  data = . %>% unnest(Predict_660) %>%  filter(Strain == "SS120", O2_uM == 25, WL == 660)) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, linetype = "dashed", data = . %>% unnest(Predict_450) %>%  filter(Strain == "SS120", O2_uM != 25, WL == 450)) +
 geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, linetype = "dashed",  data = . %>% unnest(Predict_660) %>%  filter(Strain == "SS120", O2_uM != 25, WL == 660)) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, data = . %>% unnest(Predict_660) %>%  filter(Strain == "MIT9313", O2_uM != 250, WL == 660))+
    geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, data = . %>% unnest(Predict_450) %>%  filter(Strain == "MIT9313", O2_uM != 250, WL == 450))+
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, linetype = "dashed", data = . %>% unnest(Predict_450) %>%  filter(Strain == "MIT9313", O2_uM == 250, WL == 450)) +
 geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5, linetype = "dashed",  data = . %>% unnest(Predict_660) %>%  filter(Strain == "MIT9313", O2_uM == 250, WL == 660)) +
 labs(y = "Chlorophyll Specific Growth Rate " ~  " ("  ~ d^-1~")", x = "PUR (µmol photons" ~m^-2 ~d^-1~")") +
  scale_x_continuous(label=scientific_10) +
  scale_y_continuous(breaks = c(0,0.25, 0.5, 0.75, 1.0), limits = c (0, 1.05)) +
