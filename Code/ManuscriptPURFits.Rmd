...
# model with dummy dark, 0 PAR,  growth rates. Also removed data if no growth occurred for an O2 and photoperiod combination so that the model including all photoperiods would not be skewed towards zero.
...
# This figure is a 3 panel figure faceted by strain in columns and O2 in rows. Too stretched.
# Fits are added that are not sig different individually,  next add fits that were sig different dynamically
```{r Plot fits, fig.height= 20, fig.width= 20 }

# scientific_10 <- function(x) {   parse(text=gsub("e\\+*", " %*% 10^", scales::scientific_format()(x))) }
# 
# 
# MED4PURFITPlot <- AllFitsCombined |>
#     filter(Strain == "MED4") |>
...
#   ggplot() +
#   geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 4, alpha = 1.0) +
#     geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 2, alpha = 0.8) +
#   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 1, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MED4")) + 
#    geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(O2_uM != 2.5, Strain == "MED4")) +
#    geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(O2_uM != 2.5, Strain == "MED4")) +
#  geom_text(aes(x = 1000, y = 0.75, label= "A     MED4 HLI"), data = . %>% filter(O2_uM == 250, Strain == "MED4"), size = 4, hjust = 0, vjust = 0) +
#   scale_x_continuous(label=scientific_10) +
#    scale_y_continuous(breaks = c(0,0.25, 0.5, 0.75, 1.0), limits = c (0, 1.05)) +
#   scale_colour_manual(values = MCMIXColours, labels = c("White LED", 660, 450)) +
#   scale_linetype_manual(values = MyLineTypes, labels = c("16 hr", "12 hr", "8 hr", "4 hr")) +
#    scale_shape_manual(values = PhotoperiodShapes)  +
#   ggh4x::facet_nested(rows = vars(OxygenLabel, O2_uM), 
#                     labeller = labeller(Strain = label_both, O2_uM = label_value, OxygenLabel = label_value)) +
...
#         axis.text.y = element_text(size = 9, color = "black", angle = 0, hjust = 1.0),
...
#        linetype = "Photoperiod lines"
#       )
# MED4PURFITPlot
# 
# SS120PURFITPlot <- AllFitsCombined |>
#     filter(Strain == "SS120") |>
#   unnest(cols = c(Data_all), names_sep = "_", keep_empty = TRUE) |>
#   mutate(PhotoperiodLabel_title = "Photoperiod (h)",
#          OxygenLabel = "Oxygen Concentration (µM)",
...
#   geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 4, alpha = 1.0) +
#     geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 2, alpha = 0.8) +
#   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 1, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "SS120")) + 
#    geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dashed", data = . %>% unnest(Predict_12) %>%  filter(O2_uM == 250, Strain == "SS120")) +
#    geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(Strain == "SS120")) +
#    geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(Strain == "SS120")) +
#  geom_text(aes(x = 1000, y = 0.75, label= "B     SS120 LLII/III"), data = . %>% filter(O2_uM == 250, Strain == "SS120"), size = 4, hjust = 0, vjust = 0) +
#   scale_x_continuous(label=scientific_10) +
#    scale_y_continuous(breaks = c(0,0.25, 0.5, 0.75, 1.0), limits = c (0, 1.05)) +
#   scale_colour_manual(values = MCMIXColours, labels = c("White LED", 660, 450)) +
#   scale_linetype_manual(values = c("dotdash","dashed", "dotted", "aa"), labels = c("16 hr", "12 hr", "8 hr", "4 hr")) +
#    scale_shape_manual(values = PhotoperiodShapes)  +
#   ggh4x::facet_nested(rows = vars(OxygenLabel, O2_uM), 
#                     labeller = labeller(Strain = label_both, O2_uM = label_value, OxygenLabel = label_value)) +
...
#         axis.title.x.bottom = element_blank(),
#         axis.text.x = element_blank(),
#         axis.text.y = element_text(size = 10, color = "black", angle = 0, hjust = 1.0),
#         strip.background=element_rect(color="grey87", fill = "grey82"),
#         # strip.background=element_rect(fill = "white"),
#         legend.key.size = unit(1, 'cm'), #change legend key size
#         legend.key.height = unit(1, 'cm'), #change legend key height
...
#    SS120PURFITPlot
# 
# MIT9313PURFITPlot <- AllFitsCombined |>
...
#   geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 4, alpha = 1.0) +
#     geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 2, alpha = 0.8) +
#   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 1, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MIT9313")) + 
#   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotdash", data = . %>% unnest(Predict_16) %>%  filter(O2_uM == 2.5, Strain == "MIT9313")) +
#    geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dashed", data = . %>% unnest(Predict_12) %>%  filter(O2_uM != 2.5, Strain == "MIT9313")) +
#    geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(O2_uM == 25, Strain == "MIT9313")) +
#    geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.5, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(Strain == "MIT9313")) +
#   labs(x = "PUR (µmol photons" ~m^-2 ~d^-1~")") +
#  geom_text(aes(x = 1000, y = 0.75, label= "C     MIT9313 LLIV"), data = . %>% filter(O2_uM == 250, Strain == "MIT9313"), size = 4, hjust = 0, vjust = 0) +
#   scale_x_continuous(breaks = c(0, 1000000,2000000, 3000000, 4000000),label=scientific_10, limits = c (-100, 5000000)) +
#    scale_y_continuous(breaks = c(0, 0.25, 0.5, 0.75, 1.0), limits = c (0, 1.05)) +
#   scale_colour_manual(values = MCMIXColours, labels = c("White LED", 660, 450)) +
#   scale_linetype_manual(values = c("solid","dotdash","dashed", "dotted", "aa"), labels = c("Pooled","16 hr", "12 hr", "8 hr", "4 hr")) +
#    scale_shape_manual(values = PhotoperiodShapes)  +
#   ggh4x::facet_nested(rows = vars(OxygenLabel, O2_uM), 
#                     labeller = labeller(Strain = label_both, WL = label_both, O2_uM = label_value, OxygenLabel = label_value)) +
#  guides(linetype = guide_legend(nrow = 4),
#         fill = "none",
#         colour = guide_legend(nrow = 4)) +
#  guides(shape = guide_legend(nrow = 4)) +
...
#           axis.text.x = element_text(size = 10, color = "black", angle = 0, hjust = 1.0),
#           axis.text.y = element_text(size = 10, color = "black", angle = 0, hjust = 1.0),
#         strip.background=element_rect(color="grey87", fill = "grey82"),
#         # strip.background=element_rect(fill = "white"),
#         legend.key.size = unit(1, 'cm'), #change legend key size
...
# Combine the 3 strains into 1 plot 
```{r combine plots}

# AllStrainPurFitsPlots <- plot_grid(MED4PURFITPlot, SS120PURFITPlot, MIT9313PURFITPlot, ncol = 1)
# 
# #create common x and y labels
# y.grob <- textGrob("Chlorophyll Specific Growth Rate " ~  " ("  ~ d^-1~")",
#                    gp = gpar(col="black", fontsize = 15), rot = 90)
# 
# #add to plot
# AllStrainPurFitsPlots <- grid.arrange(arrangeGrob(AllStrainPurFitsPlots, left = y.grob))

```


```{r Pur plot strain faceted in columns}

scientific_10 <- function(x) {   parse(text=gsub("e\\+*", " %*% 10^", scales::scientific_format()(x))) }


AllStrainPurFitsPlots <- AllFitsCombined |>
  unnest(cols = c(Data_all), names_sep = "_", keep_empty = TRUE) |>
  mutate(PhotoperiodLabel_title = "Photoperiod (h)",
         OxygenLabel = "Oxygen Concentration (µM)",
         Photoperiod = factor(Data_all_Photoperiod, levels = c(16, 12, 8, 4)),
         ...
         Strain = factor(Strain, levels = c("MED4", "SS120", "MIT9313"))) %>%
  ggplot() +
  geom_line(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, linetype = as.factor(Photoperiod)), size = 1) +
  ...
  geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 3, alpha = 1.0) +
    geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 1.0, alpha = 0.8) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.80, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MED4")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(O2_uM != 2.5, Strain == "MED4")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(O2_uM != 2.5, Strain == "MED4")) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.80, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "SS120")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "dotdash", data = . %>% unnest(Predict_16) %>%  filter(O2_uM == 2.5, Strain == "SS120")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "dashed", data = . %>% unnest(Predict_12) %>%  filter(O2_uM == 250, Strain == "SS120")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(Strain == "SS120")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(Strain == "SS120")) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.80, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MIT9313")) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "dotdash", data = . %>% unnest(Predict_16) %>%  filter(O2_uM == 2.5, Strain == "MIT9313")) +
   ...
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "dotted", data = . %>% unnest(Predict_8) %>%  filter(O2_uM != 2.5, Strain == "MIT9313")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 0.6, linetype = "aa", data = . %>% unnest(Predict_4) %>%  filter(Strain == "MIT9313")) +
 ...
 # guides(shape = guide_legend(nrow = 1)) +
  theme_bw() +
   theme( strip.text = element_text(size = 25),
        ...
        legend.text = element_text(size = 29),   #change legend text font size
        legend.position = "top",
...
# This labels each pane with a letter but turns of the dacet labels
# AllStrainPurFitsPlots <- tag_facet(AllStrainPurFitsPlots, open = "", close = "", tag_pool = LETTERS, size = 8)
...
ggsave(file = file.path(PlotsOut, paste("AllStrainPurFitsPlots",".png",sep = "")), plot = AllStrainPurFitsPlots, device = NULL, scale = 1, height=30, width= 44, units = c("cm"),dpi = 300, limitsize = TRUE)

ggsave(file = file.path(PlotsOut, paste("AllStrainPurFitsPlots",".tiff",sep = "")), plot = AllStrainPurFitsPlots, device = NULL, scale = 1, height=30, width= 44, units = c("cm"),dpi = 300, limitsize = TRUE)
...
# model with dummy dark, 0 PAR,  growth rates. Also removed data if no growth occurred for an O2 and photoperiod combination so that the model including all photoperiods would not be skewed towards zero.
StrainO2NestModels <- PURPARFits_dummy |>
  nest(data = -c("Strain","O2_uM")) |>
  mutate(GRCplatt_Model = map(data, possibly(~nlsLM(deltaOD_Lmu_corr_day ~ grcplatt(I = PURPhotonDose_day, a, b, Pmax),
                              data = .x,
                              start = list(a = 1e-7, b = 1e-8, Pmax = 1), 
...
StrainO2NestModels <- full_join(StrainO2NestModels, AllData)
...
# Join  
StrainO2WL_AllModels <- bind_rows(KeepPlattModel, KeepLmModel) 
...
# O2 and WL nested data frame containing the fits for each WL for each strain
 StrainO2WL_PlattLmModel <- StrainO2WL_AllModels |>
...
# O2 nested data frame containing the single fit of pooled WL
 #filter to keep platt models
KeepPlattModel <- StrainO2NestModels|>
  select(-c(GRClm_Glance, GRClm_Model, GRClm_Param, GRClm_Predict)) |>
   filter(!map_lgl(GRCplatt_Model, is.null))

#filter to keep only the models that failed to fit a platt model, fits by lm remains. 
KeepLmModel <- StrainO2NestModels|>
  filter(map_lgl(GRCplatt_Model, is.null)) |>
 select(-c(GRCplatt_Glance, GRCplatt_Model, GRCplatt_Param, GRCplatt_Predict))
...
# O2 nested data frame containing the single fit of pooled WLs 
 StrainO2_PlattLmModel <- StrainO2_AllModels|>
  # select(-c(data)) |>
  pivot_longer(cols = -c(Strain, O2_uM, data, Data_all)) |>
  drop_na(value) |>
  filter(str_detect(name, pattern = "Model") | str_detect(name, pattern = "Glance") | str_detect(name, pattern = "Param") | str_detect(name, pattern = "Predict")) |>
...
# ANOVA comparison of red light to blue light for each strain and [O2]. 
...
AllFitsCombinedWL |>
  ...
  mutate(PhotoperiodLabel_title = "Photoperiod (h)",
         ...
         Strain = factor(Strain, levels = c("MED4", "SS120", "MIT9313"))) %>%
  ggplot() +
  geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 2, alpha = 0.7) +
    geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 1.0, alpha = 0.5) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 1.0, alpha = 0.3, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MED4")) + 
   geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, data = . %>% unnest(Predict_450) %>%  filter(O2_uM == 2.5, Strain == "MED4",  WL == 450)) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, data = . %>% unnest(Predict_660) %>%  filter(Strain == "MED4", WL == 660))+
    geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, data = . %>% unnest(Predict_WW) %>%  filter(Strain == "MED4", WL == "WW")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 1.0, alpha = 0.3, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "SS120")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6,  data = . %>% unnest(Predict_450) %>%  filter(Strain == "SS120", O2_uM != 25, WL == 450)) +
 geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6,  data = . %>% unnest(Predict_WW) %>%  filter(Strain == "SS120", O2_uM != 250, WL == "WW")) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted), size = 1.0, alpha = 0.3, linetype = "solid", data = . %>% unnest(Predict_pooled) %>%  filter(Strain == "MIT9313")) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, data = . %>% unnest(Predict_660) %>%  filter(Strain == "MIT9313", O2_uM != 250, WL == 660))+
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6,  data = . %>% unnest(Predict_WW) %>%  filter(Strain == "MIT9313", O2_uM == 2.5, WL == "WW")) +
 labs(title = "Anova fit comparisons of WL to pooled", y = "Chlorophyll Specific Growth Rate " ~  " ("  ~ d^-1~")", x = "PUR (µmol photons" ~m^-2 ~d^-1~")") +
  scale_x_continuous(label=scientific_10) +
  scale_y_continuous(breaks = c(0,0.25, 0.5, 0.75, 1.0), limits = c (0, 1.05)) +
  scale_colour_manual(values = MCMIXColours, labels = c("White LED", 660, 450)) +
  # scale_linetype_manual(values = MyLineTypes, labels = c("16 hr", "12 hr", "8 hr", "4 hr")) +
...
# This labels each pane with a letter but turns of the facet labels
# AllStrainPurFitsPlots <- tag_facet(AllStrainPurFitsPlots, open = "", close = "", tag_pool = LETTERS, size = 8)
...
BluevsRedPurFitsPlots <- AllFitsCombinedWL |>
  ...
  geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_ReplicateMean_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 3, alpha = 1.0) +
    geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Photoperiod), color = as.factor(WL), fill = as.factor(WL)), size = 1.0, alpha = 0.8) +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, data = . %>% unnest(Predict_450) %>%  filter(Strain == "MED4",  WL == 450), linetype = "solid", show.legend = T) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, data = . %>% unnest(Predict_660) %>%  filter(Strain == "MED4", WL == 660) , linetype = "solid") +
   geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6,  data = . %>% unnest(Predict_450) %>%  filter(Strain == "SS120", O2_uM == 25, WL == 450) , linetype = "solid") +
 geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6,  data = . %>% unnest(Predict_660) %>%  filter(Strain == "SS120", O2_uM == 25, WL == 660) , linetype = "solid") +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, linetype = "dashed", data = . %>% unnest(Predict_450) %>%  filter(Strain == "SS120", O2_uM != 25, WL == 450), show.legend = T) +
 geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, linetype = "dashed",  data = . %>% unnest(Predict_660) %>%  filter(Strain == "SS120", O2_uM != 25, WL == 660)) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, data = . %>% unnest(Predict_660) %>%  filter(Strain == "MIT9313", O2_uM != 250, WL == 660) , linetype = "solid")+
    geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, data = . %>% unnest(Predict_450) %>%  filter(Strain == "MIT9313", O2_uM != 250, WL == 450) , linetype = "solid")+
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, linetype = "dashed", data = . %>% unnest(Predict_450) %>%  filter(Strain == "MIT9313", O2_uM == 250, WL == 450)) +
 geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.6, linetype = "dashed",  data = . %>% unnest(Predict_660) %>%  filter(Strain == "MIT9313", O2_uM == 250, WL == 660)) +
 labs(y = "Chlorophyll Specific Growth Rate " ~  " ("  ~ d^-1~")", x = "PUR (µmol photons" ~m^-2 ~d^-1~")") +
  scale_x_continuous(label=scientific_10) +
  scale_y_continuous(breaks = c(0,0.25, 0.5, 0.75, 1.0), limits = c (0, 1.05)) +
  scale_colour_manual(values = MCMIXColours, labels = c(660, 450)) +
  scale_linetype_manual(values = c("solid", "dashed"), labels = c("Significantly different", "Not significantly different")) +
  scale_shape_manual(values = PhotoperiodShapes)  +
  ggh4x::facet_nested(rows = vars(OxygenLabel, O2_uM), cols = vars(Strain), 
                    labeller = labeller(Strain = label_value, O2_uM = label_value, OxygenLabel = label_value)) +
...
ggsave(file = file.path(PlotsOut, paste("BluevsRedPurFitsPlots",".png",sep = "")), plot = BluevsRedPurFitsPlots, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)

# ggsave(file = file.path(PlotsOut, paste("BluevsRedPurFitsPlots",".tiff",sep = "")), plot = BluevsRedPurFitsPlots, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)
...
knitr::kable(WLAnovaTable,
             caption = "pvalues for each ANOVA comparison of fit of red and blue light in each O~2~ experiment for each strain.",
            col.names =  c("Strain",
                           "[O~2~] (µM)",
                          "WL1",
                          "WL2",
