...
NoFilter <- PURPARFits_dummy |>
 ...
 # Join nested df with all data to df containing filtered data 
  PURPARStrainO2NestModels_filtered <- full_join(PURPARStrainO2NestModels_filtered, NoFilter) %>% 
...
UnnestTest <- PURPARStrainO2NestModels_filtered %>% 
  # unnest(Data_all, names_sep = "_", keep_empty = TRUE) %>% 
  unnest(GRCplatt_Predict, keep_empty = TRUE)
...
ForFitPlots <- bind_rows(Unnest_all, Unnest_16, Unnest_12, Unnest_8, Unnest_4)

remove(Unnest_16, Unnest_12, Unnest_8, Unnest_4, Unnest_all)
...
ForFitPlots |>
  ...
  geom_line(aes(x = PURPhotonDose_day, y = .fitted), data = . %>% filter(FitCategory == "Pooled"), size = 1) + 
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, linetype = "twodash"), data = . %>% filter(FitCategory == "Photoperiod16" & O2_uM == 25), size = 0.5) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted, linetype = "dotdash"), data = . %>% filter(FitCategory == "Photoperiod8"), size = 0.5) +
  geom_line(aes(x = PURPhotonDose_day, y = .fitted,  linetype = "aa"), data = . %>% filter(FitCategory == "Photoperiod4"), size = 0.5) +
  # geom_line(aes(x = PURPhotonDose_day, y = .fitted, shape = as.factor(Photoperiod), linetype = as.factor(Photoperiod)), data = . %>% filter(FitCategory == "Photoperiod4"), size = 0.5) +
 ...
 #geom_text(aes(x = PURPhotonDose_day, y = deltaOD_Lmu_corr, label= Run),hjust=0.3, vjust=0) +
  ...
  # scale_y_continuous(limits = c(-0.05,0.050)) +
   scale_linetype_manual(values = c("solid", "twodash", "dotted","dotdash", "aa"),labels = c("Pooled", "16 hr", "12 hr", "8 hr", "4 hr")) +
   scale_shape_manual(values = PhotoperiodShapes)  +
...
StrainO2filteredPhotoperiod_PlattLmModel |>
  ...
  geom_line(aes(x = PURPhotonDose_day, y = `.fitted`)) + 
   geom_text(aes(x = 500000, y = 0.75, label = paste(ModelType), show.legend = FALSE)
     ...
     scale_colour_manual(values = MCMIXColours) +
  facet_grid(rows = vars(O2_uM, Photoperiod), cols = vars(Strain)) +
    labs(title = "Photoperiods included in the single fit for each [O2]") +
  theme_bw()
...
# ANOVA on new model with significantly different fits removed from the single fit.
```{r compare 1 fit of all photoperiod to each photoperiods using platt or lm}
...
AllPhotoperiods <- StrainO2filteredPhotoperiod_PlattLmModel %>% 
  select(Strain, O2_uM, Model, Predict) %>% 
 rename(All = 'Model',
        Predict_all = 'Predict')

AllFitsCombined_filtered <- left_join(Wider_Photoperiods, AllPhotoperiods)


ANOVA_AllPhotoperiods <- AllFitsCombined_filtered %>% 
  mutate(twelve = `12`,
         four = `4`,
         eight = `8`,
         sixteen = `16`) %>%
