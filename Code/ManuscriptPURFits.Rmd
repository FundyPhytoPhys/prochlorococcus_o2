...
AllPhotoperiods <- StrainO2filtered_PlattLmModel %>% 
  select(Strain, O2_uM, Model, Predict, data) %>% 
 rename(All = 'Model',
        Predict_all = 'Predict',
        Data_all = 'data')
...
ANOVA_AllPhotoperiods %>% 
  ...
  labs(title = "ANOVA compare all models",
       ...
       dot = significantly different from all photoperiod model (p value < 0.05)")
...
AllFitsCombined |>
  ...
  mutate(PhotoperiodLabel_title = "Photoperiod (h)",
         ...
         O2_uM = factor(O2_uM, levels = c(250, 25, 2.5))) %>%
  ggplot() +
  geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Data_all_Photoperiod), color = as.factor(Data_all_WL), fill = as.factor(Data_all_WL)), size = 6, alpha = 1.0) +
    geom_point(aes(x = Data_all_PURPhotonDose_day, y = Data_all_deltaOD_Lmu_corr_day, shape = as.factor(Data_all_Photoperiod), color = as.factor(Data_all_WL), fill = as.factor(Data_all_WL)), size = 1, alpha = 0.8) +
