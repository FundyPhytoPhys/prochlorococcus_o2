...
AllFitsTest <- AllFitsCombined %>%
  # unnest(Data_all) %>% 
  tidyr::unnest(Predict_4, names_sep = "_", keep_empty = TRUE) %>% 
  tidyr::unnest(Predict_8, names_sep = "_", keep_empty = TRUE) %>%
  tidyr::unnest(Predict_12, names_sep = "_", keep_empty = TRUE) %>%
  tidyr::unnest(Predict_16, names_sep = "_", keep_empty = TRUE) %>%


  rename(Four_PURPhotonDose_day = PURPhotonDose_day,
         Four_deltaOD_Lmu_corr_day = deltaOD_Lmu_corr_day,
         Four_Fitted = .fitted) %>% 
  select(-c(.resid)) %>%
  unnest(Predict_8) %>% 
  rename(Eight_PURPhotonDose_day = PURPhotonDose_day,
         Eight_deltaOD_Lmu_corr_day = deltaOD_Lmu_corr_day,
         Eight_Fitted = .fitted) %>% 
  select(-c(.resid)) 
...
AllFitsCombined |>
   filter(Strain == "MED4") |>
  unnest(cols = c(Data_all, Predict_pooled), names_sep = "_", keep_empty = TRUE) |>
  ...
  geom_line(aes(x = Predict_pooled_PURPhotonDose_day, y = Predict_pooled_.fitted), size = 1) + 
  geom_line(aes(x = Predict_4_PURPhotonDose_day, y = Predict_4_.fitted, shape = as.factor(Predict_4_Photoperiod), linetype = as.factor(Predict_4_Photoperiod)), size = 0.5) +
  geom_line(aes(x = Predict_8_PURPhotonDose_day, y = Predict_8_.fitted, shape = as.factor(Predict_8_Photoperiod), linetype = as.factor(Predict_8_Photoperiod)), size = 0.5) +
  geom_line(aes(x = Predict_12_PURPhotonDose_day, y = Predict_12_.fitted, shape = as.factor(Predict_12_Photoperiod), linetype = as.factor(Predict_12_Photoperiod)), size = 0.5) +
  geom_line(aes(x = Predict_16_PURPhotonDose_day, y = Predict_16_.fitted, shape = as.factor(Predict_16_Photoperiod), linetype = as.factor(Predict_16_Photoperiod)), size = 0.5) +
