...
# ggsave(file = file.path(PlotsOut, paste("MED4PURFITPlot",".png",sep = "")), plot = MED4PURFITPlot, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)
# 
# ggsave(file = file.path(PlotsOut, paste("SS120PURFITPlot",".png",sep = "")), plot = SS120PURFITPlot, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)
# 
# ggsave(file = file.path(PlotsOut, paste("MIT9313PURFITPlot",".png",sep = "")), plot = MIT9313PURFITPlot, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)
# 
# ggsave(file = file.path(PlotsOut, paste("MED4PURFITPlot",".tiff",sep = "")), plot = MED4PURFITPlot, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)
# 
# ggsave(file = file.path(PlotsOut, paste("SS120PURFITPlot",".tiff",sep = "")), plot = SS120PURFITPlot, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)
# 
# ggsave(file = file.path(PlotsOut, paste("MIT9313PURFITPlot",".tiff",sep = "")), plot = MIT9313PURFITPlot, device = NULL, scale = 1, height=25, width= 40, units = c("cm"),dpi = 300, limitsize = TRUE)
...
BluevsRedPurFitsPlots <- AllFitsCombinedWL |>
 ...
 #  geom_line(aes(x = PURPhotonDose_day, y = .fitted, color = as.factor(WL)), size = 0.5,  data = . %>% unnest(Predict_WW) %>%  filter(Strain == "MIT9313", O2_uM == 2.5, WL == "WW")) +
 labs(title = "Sig diff fits are indicated by the fit drawn. No lines, no diff bt 660 & 450", y = "Chlorophyll Specific Growth Rate " ~  " ("  ~ d^-1~")", x = "PUR (µmol photons" ~m^-2 ~d^-1~")") +
  scale_x_continuous(label=scientific_10) +
  scale_y_continuous(breaks = c(0,0.25, 0.5, 0.75, 1.0), limits = c (0, 1.05)) +
  scale_colour_manual(values = MCMIXColours, labels = c("White LED", 660, 450)) +
  # scale_linetype_manual(values = MyLineTypes, labels = c("16 hr", "12 hr", "8 hr", "4 hr")) +
