...
Implement logistic and other growth curve fits to MultiCulti growth trajectories. Individually tidied Muticultivator runs are imported from "Data/ImportedData/ImportedMCData" folder, processed and saved as .Rds in the "Data/ProcessedData/ProcessedMCData" folder. This script calculates deltaOD and filters data after x hours of decline growth from maximum in OD720 or deltaOD prior to implementing growth curve fits. 
...
ReplaceOD <- 0.0001 # replace  OD720 and/or deltaOD when log of original deltaOD results in NAN or infinite #

TruncFactor = 0.5 # fraction of max OD for truncating full growth trajectory; used to compare mu.


RollWindow = 20 # used to generate rolling averages: measurements every 5 min; 20 points is 1.67 h 
...
MyWavelengths = c(405, 450, 470, 530, 620, 660, 730, "WW") # Multicultivator available wavelengths 
MCMIXColours = c(c(w_length2rgb(405), w_length2rgb(450), w_length2rgb(470), w_length2rgb(530),            w_length2rgb(620), w_length2rgb(660), w_length2rgb(730), "black")) # set colors specific to wavelength using the'photobiologyWavebands' package
...
Exported from runs of Import_MCData.Rmd. Run only first time in session
...
```{r read ProcessFile}
ProcessFile <- "../Data/ImportedData/ImportedMCData/20220824_PICO_MC257_RUN86_TargetDataMetaFilter.Rds"
...
ProcessData <- ProcessData %>%
  ...
  mutate(#log of zero results in -infinite #'s. Replace with 0.0001. Replacing with 0 creates very low (10e-20) rollmeans which results in very high log normalized values further on in this chunk. The following mutuate only replaces deltaOD if needed and not OD720 because deltaOD values may be very small at start of Multicultivator runs and may initially drift < 0 while OD720 remains > 0. Without replacing these negative deltaOD values, both OD720 and deltOD data points will be filtered out. 
...
ProcessData <- ProcessData %>%
  ...
  mutate(rollmeanOD720 = zoo::rollmean(OD720, k = RollWindow, align = "left", fill = NA),
         ...
         lognormOD720 = if_else(OD720 == (min(rollmeanOD720, na.rm = TRUE)), ReplaceOD, 
                                log(OD720/min(rollmeanOD720, na.rm = TRUE))),
         lognormdeltaOD = if_else(deltaOD == (min(rollmeandeltaOD, na.rm = TRUE)), ReplaceOD, 
                                  log(deltaOD/min(rollmeandeltaOD, na.rm = TRUE)))) %>%
...
ProcessDataNestGrowth <- ProcessDataNestGrowth %>%
  ...
  mutate(OD720trunc_TimetoMax = as.numeric(ifelse(OD720trunc_TimetoMax == "NULL" , NA, OD720trunc_TimetoMax))
   ...
   mutate(deltaOD_logistic = map(tubedata, possibly(~nlsLM(deltaOD ~ logistic_eqn(x = time, Lmax, Lmu, Lintercept),
                            data = .x,
                            start = list(Lmax = max(.$rollmeandeltaOD, na.rm = TRUE),
                                         Lmu = (log(max(.$rollmeandeltaOD, na.rm = TRUE)) - log(min(.$rollmeandeltaOD, na.rm =  TRUE)))/max(.$time),
                                         Lintercept = min(.$rollmeandeltaOD, na.rm = TRUE)),
                            control = list(maxiter = 500)), otherwise = NULL)),
  ...
  mutate(deltaODtrunc_TimetoMax = as.numeric(ifelse(deltaODtrunc_TimetoMax == "NULL" , NA, deltaOD_TimetoMax))
   ...
   mutate(GrowthAmpdeltaOD = if_else(deltaOD_TimetoMax <= MaxAmpHour | is.na(deltaOD_TimetoMax),
                                     map(tubedata, ~growth_amp_decline(y = .$rollmeandeltaOD)),
                                     map(tubedata, ~growth_amp(y = .$rollmeandeltaOD))),
          GrowthAmpOD720 = if_else(OD720_TimetoMax <= MaxAmpHour | is.na(OD720_TimetoMax),
                                   map(tubedata, ~growth_amp_decline(y = .$rollmeanOD720)),
                                   map(tubedata, ~growth_amp(y = .$rollmeanOD720)))
         )
...
ProcessDataNestGrowth2 <- ProcessDataNestGrowth %>%
  ...
  nest(tubedata = c(time, ToD, Day, Actinic_par, OD680, OD720, deltaOD, ActinicMid_parOD720,
   ...
   mutate(GrowthAmpdeltaOD_14days = if_else(deltaOD_TimetoMax <= MaxAmpHour | is.na(deltaOD_TimetoMax),
                                            map(tubedata, ~growth_amp_decline(y = .$rollmeandeltaOD)),
                                            map(tubedata, ~growth_amp(y = .$rollmeandeltaOD))),
          GrowthAmpOD720_14days = if_else(OD720_TimetoMax <= MaxAmpHour| is.na(OD720_TimetoMax),
                                          map(tubedata, ~growth_amp_decline(y = .$rollmeanOD720)),
                                          map(tubedata, ~growth_amp(y = .$rollmeanOD720))),
