...
ProchlorococcusEnzymeKms <- MergedData %>%
  ...
  # geom_ribbon(aes(ymin = log10(5e-4), ymax =  log10(5e-3), x = as.numeric(reorder(EnzymeName, Km_mM))), alpha = 0.3, fill = "blue") +
  # geom_ribbon(aes(ymax = log10(5e-2), ymin =  log10(5e-3), x = as.numeric(reorder(EnzymeName, Km_mM))), alpha = 0.3, fill = "green") +
  # geom_ribbon(aes(ymax = log10(280e-3), ymin =  log10(230e-3), x = as.numeric(reorder(EnzymeName, Km_mM))), alpha = 0.3, fill = "#FF0000") +
  ...
  # geom_hline(yintercept = log10(5e-4), linetype = "dotted") +
  # geom_hline(yintercept = log10(5e-3), linetype = "dotted") +
  # geom_hline(yintercept = log10(5e-3), linetype = "dotted", color = "brown") +
  # geom_hline(yintercept = log10(5e-2), linetype = "dotted", color = "brown") +
  # geom_hline(yintercept = log10(230e-3), linetype = "dotted", color = "red") +
  # geom_hline(yintercept = log10(290e-3), linetype = "dotted", color = "red") +
  # scale_color_gradient(low = "#0980F6", high = "#F67F09", na.value = "black") +
...
```{r, DiatomEnzymeKms, fig.width = 20, fig.height = 20}
DiatomEnzymeKms <- MergedData %>%
  filter(!is.na(Gene_count),
         Taxa == "Diatom",
         !is.na(Estimated)) %>%
  # mutate(StrainLabel = case_when(Strain == "CCMP1375" ~ "SS120",
  ...
  mutate(Km_mM_min = min(Km_mM, na.rm = TRUE), 
         ...
         Gene_count = as.character(Gene_count)) %>%
  ungroup() %>%
  mutate(Latitude = case_when(species == "multistrata" & Strain == "B856" ~ 40.485, #https://doi.org/10.1111/nph.14557
                              species == "inconspicua" & Strain == "GAI-293" ~ 21.57, #https://doi.org/10.1038/s41598-021-95106-3
                              species == "robusta" & Strain == "D6" ~ 51.01, #https://bccm.belspo.be/catalogues/dcg-strain-details?ACCESSION_NUMBER=DCG%200498&LIST1=&TEXT1=&LIST2=&TEXT2=&LIST3=&TEXT3=&LIST4=&TEXT4=&LIST5=&TEXT5=&CONJ=
                              TRUE ~ Latitude)) %>% 
  mutate(Polar = case_when(Latitude > 59 ~ "Polar",
                           Latitude < -59 ~ "Polar",
                           is.na(Latitude) ~ "Unknown",
                           TRUE ~ "Non Polar")) %>% 
  filter(Polar != "Unknown") %>% 
  filter(Genus %notin% c("Chaetoceros", "Amphora", "Extubocellulus"),
         species %notin% c("minuscula")) %>% #select(Genus, species, Strain, Polar, Ome) %>% unique()
  ggplot() +
  geom_errorbar(aes(x = reorder(EnzymeName, Km_mM), ymin = log10(Km_mM_min), ymax = log10(Km_mM_max))) +
  # geom_ribbon(aes(ymin = log10(5e-4), ymax =  log10(5e-3), x = as.numeric(reorder(EnzymeName, Km_mM))), alpha = 0.3, fill = "blue") +
  # geom_rect(ymin = log10(5e-4), ymax =  log10(5e-3), xmin = -Inf, xmax = Inf, alpha = 0.03, fill = "blue") +
  annotate(geom = "rect", ymin = log10(5e-4), ymax =  log10(5e-3), xmin = -Inf, xmax = Inf, alpha = 0.3, fill = "blue") +
  annotate(geom = "rect", ymax = log10(5e-2), ymin =  log10(5e-3), xmin = -Inf, xmax = Inf, alpha = 0.3, fill = "green") +
  annotate(geom = "rect", ymax = log10(280e-3), ymin =  log10(230e-3), xmin = -Inf, xmax = Inf, alpha = 0.3, fill = "#FF0000") +
  # geom_ribbon(aes(ymax = log10(5e-2), ymin =  log10(5e-3), x = as.numeric(reorder(EnzymeName, Km_mM))), alpha = 0.3, fill = "green") +
  # geom_ribbon(aes(ymax = log10(280e-3), ymin =  log10(230e-3), x = as.numeric(reorder(EnzymeName, Km_mM))), alpha = 0.3, fill = "#FF0000") +
  geom_point(aes(x = reorder(EnzymeName, Km_mM), y = log10(Km_mM), color = Gene_count, shape = as.logical(Estimated)), size = 5, stroke = 2) +
  geom_text(aes(x = 16.5, y = -2.8, label = "[O2] ≈ 2.5 µM"), color = "blue", size = 6) +
  geom_text(aes(x = 16.5, y = -1.8, label = "[O2] ≈ 25 µM"), color = "darkgreen", size = 6) +
  geom_text(aes(x = 16.5, y = -0.35, label = "[O2] ≈ 250 µM"), color = "#FF0000", size = 6) +
  geom_vline(aes(xintercept = reorder(EnzymeName, Km_mM), color = Gene_count), size = 5,  data = . %>% filter(is.na(Km_mM)), linetype = "solid") +
  ggh4x::facet_nested(rows = vars(Polar, interaction(Genus, species))) +
  theme_bw() +
  ggpubr::rotate_x_text(angle = 80) +
  scale_shape_manual(values = c(19,8), labels = c("FALSE" = "Literature", "TRUE" = "Predicted")) +
  scale_color_brewer(palette="Dark2") +
  # labs(title = "Affinities for Phytoplankton Enzymes for Oxygen", 
  #      x = "Enzyme Name",
  #      y = ~log[10] ~ "("~K[m] ~O[2] ~"(mM))",
  #      colour = "Gene Count",
  #      shape = ""~K[m]) +
   theme(axis.title = element_text(size = 40, color = "black"),
         title = element_text(size = 30),
...
```{r, DiatomEnzymeKms ggsave, include = FALSE} 
ggsave(filename = file.path(Figures, "DiatomEnzymeKms.png"), plot = DiatomEnzymeKms, height = 37, width = 20)
```
...
```{r DiatomLightEnzymesFig, echo = TRUE, fig.height = 10, fig.width = 15}
DiatomBothOmes <- MetaData %>%
  mutate(FullName = interaction(Genus, species, Strain, sep = " ")) %>% 
  filter(Taxa == "Diatom") %>% 
  select(Ome, Genus, species, Strain, FileName) %>% 
  pivot_wider(names_from = Ome, values_from = FileName) %>% 
  mutate(BothOmesPresent = case_when(#!is.na(Genome) ~ "GenomePresent",
                                     #!is.na(Transcriptome) ~ "TranscriptomePresent",
                                     !is.na(Genome) & !is.na(Transcriptome) ~ "BothPresent")) %>% 
  pivot_longer(cols = c(Genome, Transcriptome), values_to = "FileName", names_to = "Ome")  %>% 
  filter(!is.na(FileName)) %>% 
  mutate(BothOmesPresentTranscriptome = case_when(BothOmesPresent == "BothPresent" & Ome == "Transcriptome" ~ 1,
                                                  TRUE ~ 0))

DiatomLightEnzymesFig <- LightEnzymesMerged %>% 
  filter(Taxa == "Diatom",
         ) %>% #Ome == "Genome"
  mutate(Compound = case_when(Compound == "blue-light regulator 2" ~ "light",
  ...
  mutate(Latitude = case_when(species == "multistrata" & Strain == "B856" ~ 40.485, #https://doi.org/10.1111/nph.14557
                              ...
                              species == "robusta" & Strain == "D6" ~ 51.01, #https://bccm.belspo.be/catalogues/dcg-strain-details?ACCESSION_NUMBER=DCG%200498&LIST1=&TEXT1=&LIST2=&TEXT2=&LIST3=&TEXT3=&LIST4=&TEXT4=&LIST5=&TEXT5=&CONJ=
                              TRUE ~ Latitude)) %>% 
  right_join(., DiatomBothOmes, by = c("FileName", "Ome", "Genus", "species", "Strain")) %>% 
    mutate(Polar = case_when(Latitude > 59 ~ "Polar",
                           Latitude < -59 ~ "Polar",
                           is.na(Latitude) ~ "Unknown",
                           TRUE ~ "Non Polar")) %>% 
  mutate(Compound = case_when(Compound == "far red light" ~ "Far red",
                                 Compound == "light" ~ "Visible",
                                 Compound == "UV light" ~ "UV"),
         Description = case_when(Description == "Activating Compound" ~ "Activator",
                                 TRUE ~ Description),
         Genus = case_when(Genus == "Conticribra_Thalassiosira" ~ "Thalassiosira",
                           TRUE ~ Genus)) %>% 
  
  filter(Polar != "Unknown") %>% 
  filter(BothOmesPresentTranscriptome != 1) %>% 
  mutate(TotalEnzyme_count = sum(Gene_count), .by = EnzymeName) %>% 
  ggplot() +
  geom_point(aes(x = fct_reorder(EnzymeName, desc(TotalEnzyme_count)), y = interaction(Genus, species, sep = " "), size = (Gene_count), color = Polar, shape = Ome)) +
  ggh4x::facet_nested(cols = vars(Description, Compound), rows = vars(Polar),  scale = "free", space = "free") + #
  
  labs(x = "Enzyme Name",
       y = "Diatom",
       size = "Gene Count",
       color = "Isolation Location") +
  # scale_size_manual(values = c(1,2,3,4), breaks)
  theme_bw() +
  ggpubr::rotate_x_text(hjust = 1, angle = 70) +
...
DiatomLightEnzymesFig
```

```{r, save DiatomLightEnzymesFig, ggsave, include = FALSE} 
ggsave(filename = file.path(Figures, "DiatomLightEnzymesFig.png"), plot = DiatomLightEnzymesFig, height = 10, width = 15)
```
...
```{r}
# Flv_BLAST_files <- list.files(path = BLAST, pattern = "*.tsv", full.names = TRUE)
```

```{r}
# Flv_BLAST <- Flv_BLAST_files %>% 
#   set_names(.) %>%
#   map_dfr(read_tsv, col_names = c("qseqid", "sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore"), .id = "FileName") %>%
#   mutate(FileName = str_remove(FileName, "../BLAST/"),
#          FileName = str_remove(FileName, "_Flv.tsv")) %>% 
#   left_join(., MetaData, by = c("FileName")) %>% 
#   group_by(Strain, qseqid, sseqid) %>% 
#   mutate(Gene_count = n()) %>% 
#   filter(evalue < 1e-10)
```

Using an evalue of 0.05 above
```{r}
# Flv_BLAST %>% 
#   mutate(StrainLabel = case_when(Strain == "CCMP1375" ~ "SS120",
#                                  Strain == "CCMP1986" ~ "MED4",
...
#   mutate(StrainLabel = factor(StrainLabel, levels = c("MED4", "NATL2A", "SS120", "MIT9313"))) %>% 
#   ggplot() +
#   geom_point(aes(x = qseqid, y = sseqid, color = log10(evalue))) +
#   ggh4x::facet_nested(rows = vars(StrainLabel), scales = "free", space = "free") +
#   theme_bw() +
