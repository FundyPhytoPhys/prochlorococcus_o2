...
poi |>
  ...
  geom_vline(xintercept = O2_uM,   linewidth = 2, alpha = 0.1) +
  geom_hline(yintercept = DepthPAR, linewidth = 2, alpha = 0.1) +
  facet_grid(rows = vars(clade), cols = vars(factor(complex, levels = c("psii", "cytb6f", "psi", "atp", "rubisco"), labels = PhotosynthLabels))) +
  labs(y = "Depth (m)",
       x = expression("O"[2] ~ "(µM)")) +
    # ) +
  theme_bw()
...
poi |>
  ...
  ggplot() +
  geom_jitter(aes(x = O2, y= depth), data = . %>% filter(strain %notin% AltStrainLabels), size = 0.5, width = 10, height = 10) +
  annotate(geom = "rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, alpha = 0.75, fill = "white") +
  geom_jitter(aes(x = O2, y= depth), data = . %>% filter(strain %in% AltStrainLabels), size = 0.5, width = 10, height = 10) +
