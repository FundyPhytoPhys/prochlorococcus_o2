...
poi |>
  ...
  ggplot() +
  geom_jitter(aes(x = O2, y= depth), data = . %>% filter(strain %notin% AltStrainLabels), size = 0.1, width = 20, height = 20) + #grey points not in AltStrainLabels
 # geom_count(aes(x = O2, y= depth)) +
  annotate(geom = "rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, alpha = 0.75, fill = "white") +
  geom_jitter(aes(x = O2, y= depth), data = . %>% filter(strain %in% AltStrainLabels), size = 0.1, width = 20, height = 20) + #black points in AltStrainLabels
...
poi |>
  ...
  filter(CladeMerge != "HLII") |>
  ggplot() +
  geom_jitter(aes(x = O2, y= depth), data = . %>% filter(strain %notin% AltStrainLabels), size = 0.1, width = 20, height = 20) +
  annotate(geom = "rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, alpha = 0.75, fill = "white") +
  geom_jitter(aes(x = O2, y= depth), data = . %>% filter(strain %in% AltStrainLabels), size = 0.1, width = 20, height = 20) +
  # geom_point(aes(x = O2, y= depth, alpha = StrainAlpha)) +
  scale_x_continuous(limits = c(0, 260)) +
  scale_y_reverse(limits = c(260, 0)) +
  geom_vline(xintercept = O2_uM,   linewidth = 2, alpha = 0.1) +
  ...
  theme(strip.text = element_text(size = 8))
   ...
   ggsave(plot = last_plot(), filename = (file.path("..", "OutPut", "PLOSRound2", "Fig10.tiff")))
