...
before noise filtering; with more sophisticated noise filtering 
...
Project <- "PICO"
DataIn <- file.path("..", "Data", "RawData", "MultiCultiData.zip")
DataOut <- file.path("..","Data", "ImportedData", "MultiCultiData")
...
TargetFile <- "20211223_PICO_MCMIX006_RUN42.csv"
...
# TargetData <- fread_plus(Flnm = TargetFile, Skip = "key")  %>%
#    select(-c(V5))
# TargetData <- fread_plus(Flnm = paste(DataIn, "20211223_PICO_MCMIX006_RUN42.csv", sep = ":"), Skip = "key")  %>%
#    select(-c(V5))
# # %>%
# #  select(-c(value)) %>%
# #   rename(value = V5)
...
#rm(TargetDataMeta)
```
# check to confirm filter and ensure there are data points for all 8 tubes. If there is a missing tube, confirm that data was filtered out because OD680 and/or OD720 <0. If this is the case run the chunk "Add dummy points if a tube has OD values less than zero" above and rerun subsequent chunks.
```{r filterdataplot}
