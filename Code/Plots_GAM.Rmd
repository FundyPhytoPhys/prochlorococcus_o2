...
p3_Red <- grid.arrange(ContourLowO2 + theme(legend.position = 'none'), ContourIntO2 + theme(legend.position = 'none'), ContourHighO2 + theme(legend.position = 'none'), legend1, legend2, legend3, layout_matrix = LayoutNiche)
...
#  for thesis
  # ggsave(file = file.path(paste(PlotsPath, sep = "/"), paste(Project, "Hypervolume_GAM", GamColor, GamStrain, "Oxy", ".png", sep = "_"), fsep = .Platform$file.sep), plot = p3_Red,  height = 10, width = 6 )

# for presentation
  # ggsave(file = file.path(paste(PlotsPath, sep = "/"), paste(Project, "Hypervolume_GAM", GamColor, GamStrain, "Oxy", ".png", sep = "_"), fsep = .Platform$file.sep), plot = p3_Red,  height = 7.6, width = 6)
...
# Plot blue light GAM, must change GAMStrain and rerun chunk to generate plot for each strain
``` {r GAM Plot Blue WL}
GamStrain = "MIT9313" # MIT9313 MED4 SS120
GamColor = "Blue"
WL_nm = 450 
...
(ContourLowO2 <- ggplot(na.omit(HyperChlPlotLow), aes(Photoperiod, Par_ue, z = fit, fill = fit)) + geom_tile() +
    ...
    #                     labeller = labeller(O2_uM = label_value, OxygenLabel = label_value)) +
    #facet_grid(cols = vars(O2_uM)) +
    labs(title = bquote("F.   " ~ .(GamStrain) ~ '  2.5 µM' ~ "[" ~ O[2]~ "]"), size = 25) + 
   theme_classic() +
   theme(axis.text=element_text(size=20),
          axis.title.x = element_text(size=20),
...
(ContourIntO2 <- ggplot(na.omit(HyperChlPlotInt), aes(Photoperiod, Par_ue, z = fit, fill = fit)) + geom_tile() +
    ...
    #                     labeller = labeller(O2_uM = label_value, OxygenLabel = label_value)) +
    #facet_grid(cols = vars(O2_uM)) +
    labs(title = bquote("E.   " ~ .(GamStrain) ~ '  25 µM' ~ "[" ~ O[2]~ "]"), size = 25) + 
  theme_classic() +
   theme(axis.text=element_text(size=20),
          axis.title.x = element_text(size=20),
           axis.title.y = element_text(size=35)))
...
(ContourHighO2 <- ggplot(na.omit(HyperChlPlotHigh), aes(Photoperiod, Par_ue, z = fit, fill = fit)) + geom_tile() +
    ...
    #                     labeller = labeller(O2_uM = label_value, OxygenLabel = label_value)) +
    #facet_grid(cols = vars(O2_uM)) +
    labs(title = bquote("D.   " ~ .(GamStrain) ~ '  250 µM' ~ "[" ~ O[2]~ "]")) +  #https://stackoverflow.com/questions/15074127/use-expression-with-a-variable-r
theme_classic() +
   theme(axis.text=element_text(size=20),
          axis.title.x = element_text(size=20),
           axis.title.y = element_text(size=35)))
...
p3_Blue <- grid.arrange(ContourLowO2 + theme(legend.position = 'none'), ContourIntO2 + theme(legend.position = 'none'), ContourHighO2 + theme(legend.position = 'none'), legend1, legend2, legend3, layout_matrix = LayoutNiche)  

# , left = textGrob("PAR (µmol photons"~ m^-2~ ~s^-1~ ")", rot = 90, vjust = 0.5, hjust = 0.45, gp=gpar(fontsize=30)), bottom = textGrob("Photoperiod (h)", gp=gpar(fontsize=20), vjust = 0.2, hjust = 0.6))

#  for thesis
  # ggsave(file = file.path(paste(PlotsPath, sep = "/"), paste(Project, "Hypervolume_GAM", GamColor, GamStrain, "Oxy", ".png", sep = "_"), fsep = .Platform$file.sep), plot = p3_Blue,  height = 10, width = 6)

# for presentation
  # ggsave(file = file.path(paste(PlotsPath, sep = "/"), paste(Project, "Hypervolume_GAM", GamColor, GamStrain, "Oxy", ".png", sep = "_"), fsep = .Platform$file.sep), plot = p3_Blue,  height = 7.6, width = 6)

```


```{r combine red and blue light GAM plots}

CombinedGAM <- plot_grid(p3_Red, p3_Blue, nrow = 1) 
...
```{r save png}
 ggsave(file = file.path(paste(PlotsPath, sep = "/"), paste(Project, "Hypervolume_GAM", GamStrain, "Oxy", ".png", sep = "_"), fsep = .Platform$file.sep), plot = LabelledCombinedGam,  height = 10, width = 10)
```
