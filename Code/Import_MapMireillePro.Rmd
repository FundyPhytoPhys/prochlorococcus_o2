...
TropicFull<-ggplot() + 
  ...
  geom_map(data = world_coordinates2, map = world_coordinates2, aes(long, lat, map_id = region), color = "white", fill = "white", size = 0.2) +
  # geom_text(data=data_textA, aes(x=180, y=120, label=label), size=7, parse=TRUE) +
  # geom_text(data=data_textTropicFull, aes(x=0, y=110, label=label), size=5, parse=TRUE) +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("MapComboPro_yeah3",".png",sep = "")), height=16, width= 24,  dpi = 300, limitsize = TRUE)
```
