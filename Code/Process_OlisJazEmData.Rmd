...
OlisJazMetaData <- OlisJazMetaData %>%
    filter(OlisAbs_nm >= 400 &
           OlisAbs_nm <= 700) %>%
  mutate(PURAbsWholeCell_Norm = MC_NormEm * AbsNorm440) %>% 
  group_by(ID, E_days) %>% 
  mutate(SumPURAbsWholeCell_Norm = sum(MC_NormEm * AbsNorm440),
         Pur_ue = round(Par_ue * (SumPURAbsWholeCell_Norm / sum(MC_NormEm))),
