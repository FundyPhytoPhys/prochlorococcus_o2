...
# Unnest the Olis whole cell absorbance data and calculate absorbance maxima for 675 nm (red) and 450 (blue) and extract nm (wavelength) at maxima 
```{r}
...
OlisSpectra_red <- OLISSpectraUnnest %>% 
  ...
  filter(OlisAbs_nm >= 575 & OlisAbs_nm <= 750) %>% 
   mutate(Red_nm = round(OlisAbs_nm[which.max(Absorbance)]),
          MaxAbs_Red = Absorbance[which.max(Absorbance)]) %>% 
  filter(OlisAbs_nm == 675) %>% 
  select(SampleID, Run, Strain, Red_nm, MaxAbs_Red, Par_ue, E_days) %>% 
     ungroup()

OlisSpectra_blue <- OLISSpectraUnnest %>% 
  group_by(SampleID, E_days) %>%
  filter(OlisAbs_nm >= 400 & OlisAbs_nm <= 460) %>% 
   mutate(Blue_nm = round(OlisAbs_nm[which.max(Absorbance)]),
          MaxAbs_Blue = Absorbance[which.max(Absorbance)])  %>% 
  filter(OlisAbs_nm == 440) %>% 
  select(SampleID, Run, Strain, Blue_nm, MaxAbs_Blue,  Par_ue, E_days) %>% 
  ungroup()

OLISSpectraFile <- OLISSpectraFile %>%
  group_by(SampleID, E_days) %>%
  nest(AbsSpectra = c(OlisAbs_nm, Absorbance)) 

 OlisSpectraMeta_red <- left_join(OLISSpectraFile, OlisSpectra_red)
...
OlisJazMetaData <- left_join(OLISSpectraFile , JazEmFile, by = c("WL", "OlisAbs_nm" = "MCEm_nm")) %>% 
