---
title: "PICO_Growth_Manuscript_Plots"
author: 
- Mireille Savoie
- Douglas A. Campbell
...
knitr::write_bib(c("tidyverse", "lubridate","knitr", "stringr", "broom", "zoo", "RColorBrewer", "grid", "glue", "bookdown"), 'RPackages.bib')
...
Make O2_uM, PAR and WL as factor levels from highest to lowest
```{r O2 levels}
...
deltaODLogGrowthPlot <- PURPARFits %>%
  ...
  mutate(PhotoperiodLabel_title = "Photoperiod (h)",
         ...
         Oxygen_label = case_when(O2_uM %in% c(2.5, 25, 250) ~ paste("[Oxygen]", O2_uM, "µM", sep = " ")),
  ...
  # facet_grid(rows = vars(O2), cols = vars(Tube, WL, Par_ue, Strain, Photoperiod)) +
   ...
   ggh4x::facet_nested(cols = vars(PhotoLabel, Oxygen_label, Tube, SampleID,  Strain, WL_label, Par_label), 
                    labeller = labeller(Tube = label_both, WL_label = label_value, WL = label_value, Strain = label_value, OxygenLabel_title = label_value,Oxygen_label = label_value, OxygenLabel = label_value, Photoperiod = label_both, Par_label = label_value, SampleID = label_both))+
...
deltaODLogGrowthPlotExpand <- PURPARFits %>%
  filter(Run == 28,
         Tube == 6) %>% 
  mutate(PhotoperiodLabel_title = "Photoperiod (h)",
         OxygenLabel_title = "Oxygen Concentration",
  ...
  ggplot() +
  geom_point(aes(x = tubedata_time, y = tubedata_deltaOD), colour = "darkgreen", size = 0.1) +
  geom_line(aes(x = tubedata_time, y = min(tubedata_deltaOD)), linetype = "dashed", size = 2) + 
   geom_line(aes(x = tubedata_time, y = max(tubedata_deltaOD)), linetype = "dashed", size =2) + 
    geom_vline(xintercept = 610, linetype = "dashed", size =2) +
 scale_colour_manual(values = MCMIXColours) +
  # geom_point(aes(x = tubedata_time, y = deltaOD_logistic_predict_.fitted), colour = "black", size = 0.1) +
  # geom_point(aes(x = tubedata_time, y = deltaOD_logistic_predict_.resid), colour = "gold", size = 0.05) +
  # geom_point(aes(x = tubedata_time, y = tubedata_Actinic_par/5000, colour = as.factor(WL)), size = 0.25, alpha = 1.0) +
   # scale_y_continuous(name="Optical Density (ΔOD)", sec.axis=sec_axis(~.*5000, name = "Actinic PAR (µE)")) +
  #coord_cartesian(xlim = c(-5, 675)) +
  #scale_x_continuous(breaks=seq(0, 675, by = 125)) +
  # facet_grid(rows = vars(O2), cols = vars(Tube, WL, Par_ue, Strain, Photoperiod)) +
   # ggh4x::facet_nested(cols = vars(Tube, Strain,Photoperiod, Oxygen_label, WL_label, Par_label), 
   #                  labeller = labeller(Tube = label_both, WL_label = label_value, WL = label_value, Strain = label_value, OxygenLabel_title = label_value,Oxygen_label = label_value, OxygenLabel = label_value, Photoperiod = label_both, Par_label = label_value))+
  annotate(geom = "text", x = maxOD_x, y = maxOD_y, label = "Maximum ΔOD", size = 7, colour = "darkgreen") +
  annotate(geom = "text", x = minOD_x, y = minOD_y, label = "Minimum ΔOD", size = 7, colour = "darkgreen") +
  annotate(geom = "text", x = time_x, y = time_y, label = "Time", size = 7, colour = "red") +
  ...
  theme( strip.text = element_text(size = 11),
        axis.title = element_text(size = 20, color = "black"),
        axis.text.x = element_text(size = 15, color = "black", angle = 00, hjust = 1.0),
        axis.text.y = element_text(size = 15, color = "black", angle = 0, hjust = 1.0),
        # strip.background=element_rect(color="grey", fill = "grey"),
        strip.background=element_rect(fill = "grey"),
        legend.key.size = unit(1, 'cm'), #change legend key size
...
deltaODGrowthRatesPlot <- PURPARFits %>%
  filter(Run == 114,
  ...
  # unnest(cols = c(tubedata, deltaOD_logistic_predict),names_sep = "_", keep_empty = TRUE) %>%
  ggplot() +
  geom_point(aes(x = Photoperiod, y = ReplicateMean_deltaOD_Lmu_corr_day, colour = as.factor(WL)), size = 5.0) +
   # geom_errorbar(aes(x = Photoperiod, ymin = deltaOD_Lmu_corr_day - deltaOD_Lmu_se_day, ymax = deltaOD_Lmu_corr_day + deltaOD_Lmu_se_day), width = 0.2 , size = 0.3, color = 'black', alpha = 0.3, data = . %>% filter(GrowthAmpdeltaODFlag_14days  == 1)) +
  scale_colour_manual(values = MCMIXColours) +
  # geom_point(aes(x = tubedata_time, y = deltaOD_logistic_predict_.fitted), colour = "black", size = 0.1) +
  # geom_point(aes(x = tubedata_time, y = deltaOD_logistic_predict_.resid), colour = "gold", size = 0.05) +
  # geom_point(aes(x = tubedata_time, y = tubedata_Actinic_par/5000, colour = as.factor(WL)), size = 0.25, alpha = 1.0) +
  ...
  theme( strip.text = element_text(size = 11),
        axis.title = element_text(size = 20, color = "black"),
        axis.text.x = element_text(size = 15, color = "black", angle = 0, hjust = 1.0),
        axis.text.y = element_text(size = 15, color = "black", angle = 0, hjust = 1.0),
        # strip.background=element_rect(color="grey", fill = "grey"),
        strip.background=element_rect(fill = "grey"),
        legend.key.size = unit(1, 'cm'), #change legend key size
...
PurParPlot <- PURPARFits %>%
 ...
 geom_line(aes(x = Par_ue, y = Pur_ue,colour = as.factor(WL)), linewidth = 0.7) +
  ...
  #geom_jitter(aes(x = Pur_ue, y = Par_ue, colour = as.factor(Strain)), size = 3, alpha = 0.5) +
  labs(y = "PUR (µE)" , x = "PAR (µE)") +
 #geom_text(aes(x = Pur_ue, y = deltaOD_Lmu_corr, label= WL), alpha = 0.5 ,hjust = 0.5, vjust=-0.5) +
 # geom_errorbar(aes(x = Pur_ue, ymin = (deltaOD_Lmu_corr - deltaOD_Lmu_se), ymax = (deltaOD_Lmu_corr + deltaOD_Lmu_se)), size = 0.3, color = 'black', data = . %>% filter(GrowthAmpdeltaODFlag_14days  == 1)) +\
   # scale_linetype_manual(values = c("MED4" = "solid", "MIT9313" = "longdash",  "SS120" = "dotted")) +
 ...
 #                     labeller = labeller(WL = label_both, Strain = label_both)) + 
...
```{r MED4 PURPAR, fig.height= 10, fig.width= 10}

MED4PurParPlot <- PURPARFits %>%
  filter(Par_ue %in% c(30, 90, 180)) %>%
  mutate(Strain = factor(Strain, levels = c("MED4", "SS120", "MIT9313"))) %>%
  filter(Strain == "MED4") %>% 
  ggplot() +
...
```{r save individual ParPurPlots}


ggsave(file = file.path(PlotsPath, paste("MED4PurParPlot",".png",sep = "")), plot = MED4PurParPlot, device = NULL, scale = 1, height=10, width= 10, units = c("cm"),dpi = 300, limitsize = TRUE)
...
# ggsave(file = file.path(PlotsPath, paste("MED4PhotobyO2PUR",".png",sep = "")), plot = MED4PhotobyO2PUR, device = NULL, scale = 1, height=25, width= 35, units = c("cm"),dpi = 300, limitsize = TRUE)
# 
# ggsave(file = file.path(PlotsPath, paste("MIT9313PhotobyO2PUR",".png",sep = "")), plot = MIT9313PhotobyO2PUR, device = NULL, scale = 1, height=25, width= 35, units = c("cm"),dpi = 300, limitsize = TRUE)
# 
# ggsave(file = file.path(PlotsPath, paste("SS120PhotobyO2PUR",".png",sep = "")), plot = SS120PhotobyO2PUR, device = NULL, scale = 1, height=25, width= 35, units = c("cm"),dpi = 300, limitsize = TRUE)
...
ggplot()+
  geom_area(aes(x = PurData_OlisAbs_nm, y = Abs_Em_Value, fill = as.factor(WL)), color = "black", 
            size = 0.5, alpha = 0.7, show.legend = FALSE, 
            data = . %>% filter(Abs_Em_Names == "PurData_PURAbsWholeCell_Norm440")) +
  geom_line(aes(x = PurData_OlisAbs_nm, y = Abs_Em_Value, color = Abs_Em_Names,  linetype = Abs_Em_Names), size = 2) +
  geom_line(aes(x = PurData_OlisAbs_nm, y = Abs_Em_Value, color = as.factor(WL),  linetype = Abs_Em_Names), size = 2.0, data = . %>% filter(Abs_Em_Names == "PurData_MC_NormEm")) +
  scale_color_manual(values = c(MCMIXColours, "PurData_PURAbsWholeCell_Norm440" = "black", "PurData_AbsNorm440" = "aquamarine4", "PurData_MC_NormEm" = "white")) +
  scale_linetype_manual(values = c("PurData_PURAbsWholeCell_Norm440" = "solid", "PurData_AbsNorm440" = "solid",  "PurData_MC_NormEm" = "dotted"),
                        labels = c("LED emission", "Whole cell abs.", "PUR")) +
  scale_fill_manual(values = MCMIXColours) +
 # guides(colour = FALSE) +
  labs(y = "Normalized Absorbance and Emission", x = "Wavelength (nm)") +
 ...
 # coord_cartesian(ylim = c (0, 0.2)) +
  ...
  geom_text(aes(x = 430, y = 1.1, label = WL_label, color = as.factor(WL)), show.legend = FALSE,
   ...
   geom_text(aes(x = 560, y = 0.95, label = paste("PAR", "=", Par_ue, "uE", sep = " "), show.legend = FALSE, ),
    # parse = TRUE,
    hjust = 0,
    vjust = 1,
    size = 6.0
  )  +
  geom_text(aes(x = 560, y = 0.85, label = paste("PUR", "=", Pur_ue, "uE",sep = " "), show.legend = FALSE, ),
    # parse = TRUE,
    hjust = 0,
    vjust = 1,
    size = 6.0
 ...
 theme(strip.text = element_blank(),
       legend.background = element_rect(color = "transparent", fill = "transparent"),
        ...
        # legend.position = c(0.94,0.96),
        legend.position = "top",
        axis.title = element_text(vjust = 1.0, size = 30),
        # strip.text.x = element_text(size = 20),
